...
class SRDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.return_coords is True:
            return patch, targetPatch, np.array(start_coords), os.path.basename(self.data.iloc[index, 3]), np.array([(self.data.iloc[index, 4]), (self.data.iloc[index, 1])]), np.array(pad[::-1])
        else:
