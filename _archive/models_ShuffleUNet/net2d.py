...
class ShuffleUNet(nn.Module):
    ...
    def __init__(self, in_ch=1, num_features=64, n_levels=4, out_ch=1, kernel_size=3, stride=1):
        ...
        # Input
        self.conv_inp = _double_conv(in_ch, filters[0], kernel_size, stride)     
        ...
        for i in range(0, n_levels):
            self.wave_down.append(_conv_decomp(filters[i], filters[i], kernel_size, stride))
            self.pix_unshuff.append(pixel_unshuffle.PixelUnshuffle(num_features * (2**i), num_features * (2**i), kernel_size, stride, d=2))       
            self.conv_enc.append(_double_conv(filters[i], filters[i+1], kernel_size, stride))     
        ...
        for i in range(n_levels-1,-1,-1):
            self.pix_shuff.append(pixel_shuffle.PixelShuffle(num_features * (2**(i+1)), num_features * (2**(i+1)) * 2, kernel_size, stride, d=2))
            self.wave_up.append(_conv_decomp(filters[i], filters[i], kernel_size, stride))
            self.convup.append(_double_conv(filters[i] * 5, filters[i], kernel_size, stride))

        #FC
        self.out = nn.Conv2d(filters[0], out_ch, kernel_size=1, stride=1, padding=0, bias=True)
