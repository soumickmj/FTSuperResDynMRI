...
def parseARGS():
    ...
    ap.add_argument("-g", "--gpu", default="2", help="GPU ID(s).") 
    ...
    ap.add_argument("-us", "--us", default='Center6p25MaskWoPad_Tri_Norm', help="Undersample.")
    ...
    ap.add_argument("-hf", "--hrfolder", default='hrCHAOS_Norm', help="HighRes (Fully-sampled) Folder.")
    ...
    ap.add_argument("-pq", "--patchqueue", type=int, default=9600, help="Patch Queue Length. Will be used if Patch Size is not None. [TorchIO]")
    ap.add_argument("-pv", "--patchpervol", type=int, default=1600, help="N Patchs Per Volume. Will be used if Patch Size is not None. [TorchIO]")
    ...
    ap.add_argument("-nrm", "--prenorm", default=False, type=bool, help="Rescale intensities beteen 0 and 1")    
...
if __name__ == "__main__" :
    ...
    if args.prenorm:
        transforms.append(tio.transforms.RescaleIntensity((0, 1)))
    else:
        print("No Norm") #TODO remove

    trainDS = createTIODS(path_gt=label_dir_path, path_corrupt=dir_path, is_infer=False, p=1, transforms=transforms)
    ...
    if bool(args.patchsize):
        ...
        trainDS, valDS = create_patchDS(trainDS, valDS, args.patchsize, args.patchqueue, args.patchpervol, args.patchstride)
