...
def parseARGS():
    ...
    # ap.add_argument("-op", "--outpath", default=r'/mnt/public/sarasaen/Data/StaticFT/ChimpAbdomen/Protocol2/newSet/', help="Path for Output.")
    ap.add_argument("-op", "--outpath", default=r'/mnt/public/sarasaen/Data/3DDynTest/FatyAbdomen/DynProtocol0/', help="Path for Output.")
    ...
    ap.add_argument("-uf", "--usfolder", default='usTestDyn2', help="Undersampled Folder.")
    ap.add_argument("-hf", "--hrfolder", default='hrTestDyn2', help="HighRes (Fully-sampled) Folder.")
    ...
    ap.add_argument("-m", "--modelname", default="usTrain_UNETNewFTdo0.0dp3upconv_Center6p25MaskWoPad_pLossL1lvl3_FT_lrdec100_eprt0.001", help="Model to Load for testing.")
            ...
            for b, (lr_imgs, hr_imgs, start_coords, files, shapes, pad) in enumerate(tqdm(test_loader)):
                ...
                for i in range(hr_imgs.shape[0]):
                    if args.patchsize[0] != -1: #TODO: implement non-iso patch-size, now only using the first element
                        ...
                        inputs[files[i]] = tmp_in[i,1,...]  #TODO make it configurable. Currently its prevTPPatch, patch
